...
// It's required to have one tss for switching back to kernel mode(system call for example)
...
void tss_init(uint32_t idx, uint32_t kss, uint32_t kesp)
{
    ...
    /* Kernel tss, access(E9 = 1 11 0 1 0 0 1)
        1   present
        11  ring 3
        0   should always be 1, why 0? maybe this value doesn't matter at all
        1   code?
